...
class EpubAccessibilityMetadataPresenter
  ...
  def a11y_format
    @solr_document['reader_ebook_format_sim'] || 'EPUB'
  end
  ...
  def a11y_accessibility_features
    ...
    @a11y_accessibility_features ||= @solr_document['epub_a11y_accessibility_feature_ssim']&.uniq
  end
  ...
  def a11y_certified_by
    @a11y_certified_by ||= if @solr_document['epub_a11y_certifier_credential_ssi']&.start_with?('http')
  ...
  def a11y_certifier_credential
    @a11y_certifier_credential ||= if @solr_document['epub_a11y_certifier_credential_ssi']&.start_with?('http')
                                     if @solr_document['epub_a11y_certified_by_ssi']&.downcase == 'benetech'
  ...
  def a11y_conforms_to
    @a11y_conforms_to ||= if @solr_document['epub_a11y_conforms_to_ssi']&.downcase&.end_with?('#wcag-a')
  ...
  def a11y_screen_reader_friendly
    return @a11y_screen_reader_friendly if @a11y_screen_reader_friendly.present?

    value = @solr_document['epub_a11y_screen_reader_friendly_ssi']
    # The "Accessibility Claims" tab will show if either an `epub` or `pdf_ebook` representative is present. Some...
    # corner-case EPUBs might not have any a11y metadata.
    ...
    @a11y_screen_reader_friendly ||= if value.blank? || value == 'unknown'
