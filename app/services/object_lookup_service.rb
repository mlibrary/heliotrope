...
class ObjectLookupService
  def self.matches_for_csv_row(row)
    # FileSets can only be found by NOID, as they:
    #  1) have not stored any "Fulcrum collection IDs" in identifiers to date. Doing so would be a big change
    #  2) have no ISBN field at all
    # ... hence the queries on the other logical branches are constrained to Monographs only
    if row['NOID']
      ...
      [ActiveFedora::Base.where(has_model_ssim: 'Monograph', press_sim: 'heb', identifier_ssim: heb_ids), "HEB IDs #{heb_ids.join(', ')}"]
    elsif row['ISBN(s)']
      ...
      if row['Press']
        [ActiveFedora::Base.where(has_model_ssim: 'Monograph', press_sim: row['Press'], isbn_numeric: clean_isbns), "ISBN(s) #{clean_isbns.join(', ')}, restricted to Press '#{row['Press']}'"]
      else
  ...
  def self.matches(id, press = nil)
    ...
    if /^[[:alnum:]]{9}$/.match?(id)
      ...
      # Identifier(s) is a multi-valued field with entries separated by a ';'
      # heb_ids = HebHandleService.heb_ids_from_identifier(row['Identifier(s)'].split(';'))
      ActiveFedora::Base.where(has_model_ssim: 'Monograph', press_sim: 'heb', identifier_ssim: id)
    elsif [10, 13].include? id.delete("^0-9").length # ISBN has 10 or 13 digits
      cleaned_isbn = clean_isbns(id).first

      # ISBNs do not necessarily uniquely identify a Monograph across all of Fulcrum, but should within a given Press
      if press
        ActiveFedora::Base.where(has_model_ssim: 'Monograph', press_sim: press, isbn_numeric: cleaned_isbn)
      else
