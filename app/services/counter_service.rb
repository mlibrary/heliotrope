...
class CounterService
  ...
  def access_type
    ...
    if Component.find_by(handle: HandleService.handle(@presenter))
      "Controlled"
    # For assets if their permissions have expired. This will happen very infrequently
    # so we're erring on the side of OA for the majority of assets
    elsif @presenter&.permissions_expiration_date.present? && Date.parse(@presenter.permissions_expiration_date) <= Date.today
      "Controlled"
    else
      "OA_Gold"
    end
