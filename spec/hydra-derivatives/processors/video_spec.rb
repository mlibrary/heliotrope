    ...
    it "is configurable" do
      expect(subject)
                 .to receive(:encode_file)
                                 .with("mp4",
                                       Hydra::Derivatives::Processors::Ffmpeg::OUTPUT_OPTIONS => "-filter_complex \"scale='iw*min(1, min(1280/iw, 720/ih))':-2\" -vcodec mpeg4 -acodec aac -strict -2 -g 30 -ac 2 -ab 192k -ar 44100",
    ...
    context "when a mp4 playback derivative is requested" do
      ...
      it "creates a derivative file" do
        expect(subject)
            .to receive(:encode_file)
                    .with("mp4",
                          Hydra::Derivatives::Processors::Ffmpeg::OUTPUT_OPTIONS => "-filter_complex \"scale='iw*min(1, min(1280/iw, 720/ih))':-2\" -vcodec libx264 -acodec aac -g 30 -ac 2 -ab 192k -ar 44100",
                          Hydra::Derivatives::Processors::Ffmpeg::INPUT_OPTIONS => "")
        subject.process
      end
    end
    ...
    context "when a webm playback derivative is requested" do
      ...
      it "creates a derivative file" do
        expect(subject)
            .to receive(:encode_file)
                    .with("webm",
                          Hydra::Derivatives::Processors::Ffmpeg::OUTPUT_OPTIONS => "-filter_complex \"scale='iw*min(1, min(1280/iw, 720/ih))':-2\" -vcodec libvpx -acodec libvorbis -g 30 -ac 2 -ab 192k -ar 44100",
                          Hydra::Derivatives::Processors::Ffmpeg::INPUT_OPTIONS => "")
        subject.process
      end
    end
