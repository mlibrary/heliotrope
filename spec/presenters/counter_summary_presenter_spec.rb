...
RSpec.describe CounterSummaryPresenter do
  ...
  describe '#initialize' do
    ...
    it 'defaults date_published to nil' do
      expect(presenter.date_published).to be_nil
    end

    context 'when date_published is provided' do
      subject(:presenter) { described_class.new(monograph_noid, '2024-02-03') }

      it 'sets date_published' do
        expect(presenter.date_published).to eq('2024-02-03')
      end
    end
  end
  ...
  describe '#counter_start_date' do
    context 'when date_published is nil' do
      subject(:presenter) { described_class.new(monograph_noid, nil) }

      it 'returns "August 2018"' do
        expect(presenter.counter_start_date).to eq('August 2018')
      end
    end

    context 'when date_published is blank or unparseable' do
      subject(:presenter) { described_class.new(monograph_noid, '<blank>') }
    ...
    context 'when date_published is before August 2018' do
      subject(:presenter) { described_class.new(monograph_noid, '2017-05-15') }
    ...
    context 'when date_published is exactly August 2018' do
      subject(:presenter) { described_class.new(monograph_noid, '2018-08-01') }
    ...
    context 'when date_published is after August 2018' do
      subject(:presenter) { described_class.new(monograph_noid, '2024-02-03') }

      it 'returns the formatted publication date' do
        expect(presenter.counter_start_date).to eq('February 2024')
      end
    end

    context 'when date_published is in September 2018' do
      subject(:presenter) { described_class.new(monograph_noid, '2018-09-10') }

      it 'returns the formatted publication date' do
        expect(presenter.counter_start_date).to eq('September 2018')
      end
    end
  end
