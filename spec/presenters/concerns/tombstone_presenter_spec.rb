...
RSpec.describe TombstonePresenter do
  ...
  describe '#tombstone_message' do
    ...
    let(:model) { instance_double(Sighrax::Model, 'model', publisher: publisher, tombstone_message: model_tombstone_message) }
    let(:publisher) { instance_double(Sighrax::Publisher, 'publisher', tombstone_message: publisher_tombstone_message) }
    let(:publisher_tombstone_message) { }
    ...
    context 'when publisher tombstone message' do
      let(:publisher_tombstone_message) { '_Publisher_ Tombstone Message' }
  ...
  describe '#tombstone_message recursive' do
    subject { presenter.tombstone_message(true) }
    ...
    it { is_expected.to eq Sighrax.platform.tombstone_message }

    context 'when publisher tombstone message' do
      let(:publisher_tombstone_message) { '_Publisher_ Tombstone Message' }

      it { is_expected.to eq publisher_tombstone_message }
