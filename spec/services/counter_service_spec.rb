...
describe CounterService do
  ...
  describe "#access_type" do
    ...
    context "with an asset with a permissions_expiration_date less than today" do
      before do
        allow(HandleService).to receive(:handle).and_return(true)
        allow(Component).to receive(:find_by).and_return(false)
        allow(presenter).to receive(:permissions_expiration_date).and_return("2010-01-27")
      end
      it "is Controlled" do
        expect(described_class.from(controller, presenter).access_type).to eq 'Controlled'
      end
