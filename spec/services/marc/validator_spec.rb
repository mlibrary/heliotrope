    ...
    context "when the marc record's doi or handle does not match a monograph in fulcrum" do
      ...
      context "when 'strict' is set to false" do
        ...
        it "returns true" do
          ...
          expect(validator.error).to eq "ERROR\t024_bad_doi.xml does not have a DOI or Handle that is in fulcrum, 024$a value is 'not-a-real-doi'. The record will be retried nightly until a match in Fulcrum is found."
        end
      end
    end
  end
