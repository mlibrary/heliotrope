describe('Util', function () {
	...
	describe('#extend', function () {
		...
		it('accepts more than 2 arguments', function () {
			cozy.Util.extend(a, {bar: 7}, {baz: 3});
	...
	describe('#bind', function () {
		it('returns the given function with the given context', function () {
			...
			var fn2 = cozy.Util.bind(fn, {foo: 'bar'});
		...
		it('passes additional arguments to the bound function', function () {
			...
			var fn2 = cozy.Util.bind(fn, foo, a, b);
	...
	describe('#stamp', function () {
		it('sets a unique id on the given object and returns it', function () {
			var a = {},
			    id = cozy.Util.stamp(a);

			expect(typeof id).to.eql('number');
			expect(cozy.Util.stamp(a)).to.eql(id);

			var b = {},
			    id2 = cozy.Util.stamp(b);
	...
	describe('#falseFn', function () {
		it('returns false', function () {
			expect(cozy.Util.falseFn()).to.be(false);
		});
	});

	describe('#formatNum', function () {
		it('formats numbers with a given precision', function () {
			expect(cozy.Util.formatNum(13.12325555, 3)).to.eql(13.123);
			expect(cozy.Util.formatNum(13.12325555)).to.eql(13.12326);
		});
	});
	...
	describe('#getParamString', function () {
		it('creates a valid query string for appending depending on url input', function () {
			...
			expect(cozy.Util.getParamString(a.obj, a.url)).to.eql(a.result);
			...
			expect(cozy.Util.getParamString(b.obj, b.url)).to.eql(b.result);
			...
			expect(cozy.Util.getParamString(c.obj, c.url)).to.eql(c.result);
		});
	});
	...
	describe('#requestAnimFrame', function () {
		it('calles a function on next frame, unless canceled', function (done) {
			...
			cozy.Util.requestAnimFrame(spy);

			cozy.Util.requestAnimFrame(function () {
				expect(this).to.eql(foo);
				done();
			}, foo);

			cozy.Util.cancelAnimFrame(spy);
		});
	});
	...
	describe('#throttle', function () {
		it('limits execution to not more often than specified time interval', function (done) {
			...
			var fn = cozy.Util.throttle(spy, 20);
	...
	describe('#splitWords', function () {
		it('splits words into an array', function () {
			expect(cozy.Util.splitWords('foo bar baz')).to.eql(['foo', 'bar', 'baz']);
		});
	});
	...
	describe('#template', function () {
		it('evaluates templates with a given data object', function () {
			...
			var str = cozy.Util.template(tpl, {
				foo: 'Vlad',
				bar: 'Dave'
			});
		...
		it('does not modify text without a token variable', function () {
			expect(cozy.Util.template('foo', {})).to.eql('foo');
		});

		it('supports templates with double quotes', function () {
			expect(cozy.Util.template('He said: "{foo}"!', {
				foo: 'Hello'
		...
		it('throws when a template token is not given', function () {
			expect(function () {
				cozy.Util.template(undefined, {foo: 'bar'});
			}).to.throwError();
		});

		it('allows underscores and dashes in placeholders', function () {
			expect(cozy.Util.template('{nice_stuff}', {'nice_stuff': 'foo'})).to.eql('foo');
			expect(cozy.Util.template('{-y}', {'-y': 1})).to.eql('1');
		});
	});

	describe('#isArray', function () {
		expect(cozy.Util.isArray([1, 2, 3])).to.be(true);
		/* eslint no-array-constructor: 0 */
		expect(cozy.Util.isArray(new Array(1, 2, 3))).to.be(true);
		expect(cozy.Util.isArray('blabla')).to.be(false);
		expect(cozy.Util.isArray({0: 1, 1: 2})).to.be(false);
	});
});
