...
export var Modal = Class.extend({
  ...
  addTo: function(reader) {
    ...
    var modalContainer = this.options.modalContainer || reader._container;
    this.modal = modalContainer.appendChild(body.children[0]);
  ...
  activate: function() {
    ...
    var openModalCloseButtons = document.querySelectorAll('.cozy-modal[aria-hidden="false"] button[data-modal-close]');
    openModalCloseButtons.forEach(function(button) {
    ...
    modals.forEach(function(modal) {
      if (modal !== this && modal.modal && modal.modal.getAttribute('aria-hidden') === 'false') {
        modal.deactivate();
      }
    }.bind(this));
  ...
  _setPanelState: function(open) {
    if (!this.options.modalContainer || !this.options.modalContainer.classList.contains('nm--panels')) {
      return;
    }

    this.options.modalContainer.setAttribute('data-modal-activated', open ? 'true' : 'false');
    var readerContainer = this.options.modalContainer.closest('#epub.too');
    if (readerContainer) {
      readerContainer.classList.toggle('panels-open', open);
