function wut(el) {
	var line = [];
	line.push(el.tagName);
	line.push(el.getAttribute('id') || '-');
	line.push(el.getAttribute('class') || '-');
	line.push(el.clientWidth);
	line.push(el.clientHeight);
	return line.join(' / ');
}
...
function bounds(el) {
	...
	widthProps.forEach(function (prop) {
		var x = parseFloat(style[prop]);
		if ( x && x > 0 ) { xtra += " / " + prop + " " + x; }
	...
	console.log("AHOY BOUNDS FN", wut(el), "/", width, height, xtra, window);
...
var Rendition = function () {
	function Rendition(book, options) {
		...
		console.log("AHOY RENDITION", this.settings.width, this.settings.height);
  ...
  */
   ...
   */
		...
		value: function renderTo(element, options) {
			...
			// console.log("AHOY RENDERTO", options, element);
...
var IframeView = function () {
	...
	_createClass(IframeView, [{
		...
		value: function render(request, show) {
			...
			console.log("AHOY POST CREATE", wut(this.iframe));
			// Fit to size of the container, apply padding
			this.size();
			console.log("AHOY POST SIZE", wut(this.iframe));
			...
			.then(function () {
				...
				console.log("AHOY POST FORMAT", wut(this.iframe));
				this.expand();
				console.log("AHOY POST EXPAND", wut(this.iframe));
		...
		value: function size(_width, _height) {
			console.log("AHOY SIZE", _width, _height, "/", this.settings.width, this.settings.height, "/", this.layout.name, this.settings.axis);
		...
		value: function lock(what, width, height) {
			...
			console.log("AHOY LOCKING", width, height);
			...
			if (what == "width" && (0, _core.isNumber)(width)) {
				...
				console.log("AHOY WIDTH", width, elBorders.width, iframeBorders.width);
				this.resize(this.lockedWidth, width); //  width keeps ratio correct
			}

			if (what == "height" && (0, _core.isNumber)(height)) {
				this.lockedHeight = height - elBorders.height - iframeBorders.height;
				console.log("AHOY HEIGHT", width, height, elBorders.height, iframeBorders.height);
		...
		value: function expand(force) {
			...
			if (this.settings.axis === "horizontal") {
				...
				console.log("AHOY EXPAND", width, this._width, this._contentWidth);
				...
				if (width != this._width) {
					...
					console.log("AHOY EXPAND MMM", width);
				}
			} // Expand Vertically
		...
		value: function contentWidth(min) {
			...
			console.log("AHOY CONTENT WIDTH", width);
			return width;
		}
	}, {
		key: "contentHeight",
		...
		value: function resize(width, height) {
			...
			if ((0, _core.isNumber)(width)) {
				console.log("AHOY WIDTH", width);
				this.iframe.style.width = width + "px";
				this._width = width;
			}

			if ((0, _core.isNumber)(height)) {
				console.log("AHOY HEIGHT", height);
			...
			console.log("AHOY RESIZE > REFRAME", width, height, "/", this.iframeBounds.width, this.iframeBounds.height);
		...
		value: function reframe(width, height) {
			...
			console.log("AHOY REFRAME!", this.elementBounds, wut(this.element));
		...
		value: function load(contents) {
			...
			this.iframe.onload = function (event) {
				console.log("AHOY ON LOAD", wut(this.iframe));
			...
			if (this.supportsSrcdoc) {
				...
				console.log("AHOY CONTENT 1", wut(this.iframe));
				...
				console.log("AHOY CONTENT 2", wut(this.iframe));
 ...
 */
	...
	_createClass(Stage, [{
		...
		value: function bounds() {
			...
			if (!this.container) {
				console.log("AHOY BOUNDS NO CONTAINER", (0, _core.windowBounds)());
				return (0, _core.windowBounds)();
			} else {
				console.log("AHOY BOUNDS CONTAINER", this.container.getBoundingClientRect());
