...
class DefaultViewManager {
	...
	afterResized(view){
		if ( view.__target ) {
			...
			if ( offset.left != view.__target.offset.left && offset.top != view.__target.offset.top ) {
								this.moveTo(offset, false, view.width());
			}
	...
	moveTo(offset, silent=true, width){
		var distX = 0,
				distY = 0;
	...
	next(){
		...
		if(next) {
			this.clear();
						// The new section may have a different writing-mode from the old section.
						this.updateLayout();
