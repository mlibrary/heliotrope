...
desc 'Output CSV file for HEB Monographs containing weird contributor values'
namespace :heliotrope do
  task heb_contributor_changes: :environment do
    # Usage: Needs a valid datetime as a parameter
    # $ bundle exec rake "heliotrope:heb_qc_csv[2018-06-08T00:00:00-05:00]"

    filename = '/tmp/heliotrope_heb_contributor_changes.csv'
    line_count = 0

    CSV.open(filename, "w") do |csv|
      csv << ['HEB ID', 'Title (link)', 'edit', 'New Contributors Value', 'Old Contributors Value']

      Monograph.where(press_sim: 'heb').each do |m|
        original_value = m.contributor.first
        next if original_value.blank?

        # if ['[', ']', '(', ')'].any? { |char| m.contributor.first&.include?(char) }
        ...
        csv << [m.identifier.find { |i| i[/^heb.*/] },
                '=HYPERLINK("' + Rails.application.routes.url_helpers.hyrax_monograph_path(m.id) + '","' + m.title.first.gsub('"', '""') + '")',
