...
namespace :heliotrope do
  task remove_derivatives_of_deleted_file_sets: :environment do
    ...
    derivatives_start_total_in_mb = `du -s -m "#{Hyrax.config.derivatives_path}"`.split(/\t/, 2).first
    ...
    if level_4_folders_deleted > 0 || empty_folders_deleted_count > 0
      derivatives_end_total_in_mb = `du -s -m "#{Hyrax.config.derivatives_path}"`.split(/\t/, 2).first
