...
namespace :heliotrope do
  task :edit_monographs_via_csv_noid, [:input_file, :user_key] => :environment do |_t, args|
    # Usage: bundle exec rails "heliotrope:edit_monographs_via_csv_noid[/path/to/monographs.csv, <user's email>]"
  ...
  def paranoid_backup(rows, path)
    ...
    Rake::Task['heliotrope:edit_monographs_output_csv_noid'].invoke(backup_file, *noids)
    return backup_file
  end
end
