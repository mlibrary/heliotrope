  ...
  class Importer
    ...
    def run(test = false)
      ...
      csv_files.each do |file|
        ...
        # map filename to app relative path assigning empty string for external resources
        attrs['files'] = attrs['files'].map do |filename|
          filename.blank? ? '' : find_file(filename)
        end
