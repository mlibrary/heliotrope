...
{
  // HELIOTROPE MODIFICATION: suppress the fallback demo PDF so the viewer
  // doesn't auto-load compressed.tracemonkey-pldi-09.pdf when no ?file= is given.
  // pdf.js 6.1.200 viewer.mjs: defaultOptions.defaultUrl originally set to
  // "compressed.tracemonkey-pldi-09.pdf"; nulled out here (same pattern as 5.x/6.0 patch).
...
class TextLayerBuilder {
  ...
  #bindMouse(end) {
    ...
    div.addEventListener("mousedown", (event) => {
      // HELIOTROPE MODIFICATION:
      // Only enter "selecting" mode when the pointer actually lands on a text
      // span (or br), never on the bare .textLayer background.  Without this
      // guard, clicking in the whitespace between paragraphs unconditionally
      // adds the "selecting" class, which moves .endOfContent to cover the
      // entire column (top:0) and lets the browser extend the selection over
      // areas that contain no text – matching Firefox's stock pdf.js behaviour
      // where you simply cannot start a drag-selection from inter-paragraph
      // whitespace.
      if (event.target !== div) {
